...
for col, depth in enumerate(gap_depths):
    ...
    axes[col].set_title(f"gap depth {depth} m", fontsize=8)
...
fig.tight_layout()
fig.savefig("../images/gap_sonde_500m.png", bbox_inches="tight", dpi=300)
# %%
for depth in gap_depths:
    print(depth)
    print("no int", np.nanmean(np.array(no_int_diff[depth]).flatten()))
    print("+-", np.nanstd(np.array(no_int_diff[depth]).flatten()))
    print("int", np.nanmean(np.array(int_diff[depth]).flatten()))
    print("+-", np.nanstd(np.array(int_diff[depth]).flatten()))
